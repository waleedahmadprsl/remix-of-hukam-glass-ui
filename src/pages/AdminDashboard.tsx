...
const AdminDashboard: React.FC = () => {
  const [stats, setStats] = React.useState({ totalOrders: 0, totalProducts: 0, totalRevenue: 0, pendingOrders: 0, todaySales: 0, weekRevenue: 0, outOfStock: 0 });
  const [revenueData, setRevenueData] = React.useState<DayRevenue[]>([]);
  ...
  const fetchAll = async () => {
    ...
    const [ordersRes, productsRes, allOrders, recentRes] = await Promise.all([
    ...
    orders.forEach((o: any) => {
      ...
      if (dayMap[d] !== undefined) dayMap[d] += Number(o.total_amount || 0);
    });
    setRevenueData(Object.entries(dayMap).map(([date, revenue]) => ({ date: date.slice(5), revenue })));
    ...
    orders.forEach((o: any) => {
      if (o.items) {
        o.items.split("\n").forEach((line: string) => {
          const match = line.match(/Item \d+: (.+?) \(Qty: (\d+)\)/);
          if (match) productCounts[match[1]] = (productCounts[match[1]] || 0) + Number(match[2]);
        });
      }
    });
    ...
    setStats({ totalOrders: ordersRes.count || 0, totalProducts: products.length, totalRevenue, pendingOrders, todaySales, weekRevenue, outOfStock });
  };
    ...
    <AdminLayout activeTab="dashboard">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4 mb-8">
          {cards.map((card, i) => (
            <motion.div key={card.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="glass-card p-4 rounded-xl">
              <div className="flex items-center justify-between mb-2">
                <p className="text-muted-foreground text-[10px] sm:text-xs font-medium">{card.label}</p>
                <card.icon className={`w-4 h-4 ${card.color}`} />
              </div>
              <h2 className="text-lg sm:text-2xl font-bold text-foreground">{card.value}</h2>
            </motion.div>
          ))}
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6 mb-8">
          ...
          <div className="lg:col-span-2 glass-card p-4 sm:p-6 rounded-2xl">
            <h3 className="font-bold text-foreground mb-4">Revenue Trend (14 Days)</h3>
